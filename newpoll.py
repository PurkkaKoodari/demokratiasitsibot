...
NP_QUESTION, NP_OPTIONS, NP_MENU = range(3)
...
async def update_menu(update: Update, text: str, reply_markup: InlineKeyboardMarkup | ForceReply | ReplyKeyboardRemove | None):
    if update.callback_query is not None:
        if reply_markup is not None and not isinstance(reply_markup, InlineKeyboardMarkup):
            await update.callback_query.edit_message_reply_markup(None)
            await update.callback_query.from_user.send_message(text, reply_markup=reply_markup, parse_mode=ParseMode.HTML)
        else:
            await update.callback_query.edit_message_text(text, reply_markup=cast(Any, reply_markup), parse_mode=ParseMode.HTML)
    else:
        await cast(Message, update.effective_message).reply_text(text, reply_markup=reply_markup or ReplyKeyboardRemove(), parse_mode=ParseMode.HTML)
...
async def newpoll_ask_options(update: Update, context: CallbackContext, lang: str, prefix=""):
    await update_menu(update, f"{prefix}Enter poll options in {lang_icons[lang]}, one per line (or /cancel)", reply_markup=ForceReply())
    cast(dict, context.user_data)["poll_lang"] = lang
    return NP_OPTIONS
...
async def newpoll_save_options(update: Update, context: CallbackContext):
    ...
    if pid is not None and other_opts is None:
        other_opts = [opt[0] for opt in db.execute(f"SELECT text{other_lang.capitalize()} FROM options WHERE pollId = ?", [pid]).fetchall()]
        pending[f"opts_{other_lang}"] = other_opts
    # number of opts mismatch?
    if other_opts is not None and len(new_opts) != len(other_opts):
        return await newpoll_ask_options(update, context, other_lang, f"<b>Different number of options than in {lang_icons[other_lang]}!</b>\n\n")
    if pid is not None:
        # editing
        return await newpoll_main_menu(update, context, pid)
    else:
...
def newpoll_create(pending: dict):
    ...
    with db:
        cur = db.cursor()
        cur.execute("INSERT INTO polls (textFi, textEn) VALUES (?, ?)", [pending["question_fi"], pending["question_en"]])
        pid = cur.lastrowid
        assert pid
        cur.executemany(
...
def newpoll_menu_text(poll, top=None, bottom=None, pending: dict = {}):
    if "opts_fi" in pending:
        ...
        opts = db.execute("SELECT textFi, textEn FROM options WHERE pollId = ? ORDER BY orderNo ASC", [poll["id"]]).fetchall()
...
async def newpoll_callback(update: Update, context: CallbackContext):
    ...
    if not action.startswith("np_edit") and action not in ("np_commit", "np_revert"):
        user_data["poll_pending"] = {}
    match action:
        case "np_edit_qfi" | "np_edit_qen" | "np_edit_ofi" | "np_edit_oen" | "np_edit" | "np_commit" if poll["status"] != "created":
        ...
        case "np_activate":
            ...
            bottom = (
                ...
                if poll["status"] == "created" else
                "<b>Are you sure you want to REOPEN this poll?</b>"
            )
            await update_menu(
                update,
...
async def newpoll_edit_menu(update: Update, context: CallbackContext, pid: int, poll: Any):
    ...
    await update_menu(
        ...
        reply_markup=InlineKeyboardMarkup([
            *(
                ([InlineKeyboardButton("Save changes", callback_data=f"np_commit:{pid}")],)
                if pending else
                ()
            ),
            [
...
async def newpoll_main_menu(update: Update, context: CallbackContext, pid: int, poll: Any = None, *, top: str | None = None, bottom: str | None = None):
    if poll is None:
        poll = db.execute("SELECT * FROM polls WHERE id = ?", [pid]).fetchone()
        if poll is None:
            await update_menu(update, "Poll not found!", reply_markup=None)
    ...
    await update_menu(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [
                ...
                if poll["status"] == "created" else
                InlineKeyboardButton("Close & Results", callback_data=f"np_close:{pid}")
                if poll["status"] == "active" else
                InlineKeyboardButton("Reopen", callback_data=f"np_activate:{pid}"),
            ],
            *(
...
newpoll_conv = ConversationHandler(
    entry_points=[
